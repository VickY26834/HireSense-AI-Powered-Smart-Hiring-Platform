...
public class AffindaAPI {
    private static final String API_KEY = "aff_36ad14a34dcc2d68ab34f2e443cfcdcb16da5ef1";
 ...
 // ✅ Extract work experience from Affinda JSON result
    ...
    public static int calculateMatchScore(String jobSkillsCsv, List<String> resumeSkills) {
        ...
        java.util.function.Function<String, List<String>> splitAndClean = (s) -> {
            if (s == null) return new java.util.ArrayList<>();
            // split by comma, semicolon, pipe, slash, or newline
            String[] parts = s.split("[,;|/\\n]+");
            java.util.List<String> out = new java.util.ArrayList<>();
            for (String p : parts) {
                String cleaned = p.trim().toLowerCase().replaceAll("\\s+", " ");
                // remove any non-word chars at ends (keeps letters, numbers, ., +, #, - , space)
     ...
     // BUILD required set from jobSkillsCsv
        java.util.Set<String> required = new java.util.HashSet<>();
        for (String token : splitAndClean.apply(jobSkillsCsv)) {
            required.add(token);
        }
        if (required.isEmpty()) {
            return 0;
     ...
     // BUILD resume set: resumeSkills may contain single CSV strings or proper tokens
        java.util.Set<String> resumeSet = new java.util.HashSet<>();
        for (String r : resumeSkills) {
            if (r == null) continue;
            // if item itself contains separators, split it too
            for (String token : splitAndClean.apply(r)) {
                resumeSet.add(token);
            }
        }
