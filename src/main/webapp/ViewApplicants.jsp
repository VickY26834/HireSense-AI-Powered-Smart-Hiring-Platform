...
<%@ page import="hiresense.pojo.ApplicationPojo, hiresense.pojo.JobPojo"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
	...
	<main class="container  py-5">
		<div class="card bg-white p-4 mb-4 border-0 shadow-sm rounded-4">
			...
			<p class="text-muted">
				...
				<i class="bi bi-currency-rupee"></i><%=job.getPackageLpa() %> &nbsp; 
				<i class="bi bi-people"></i><%=job.getVacancies() %>
			</p>
		</div>
					...
					<div class="col-md-4">
						...
						<select
							...
							<option value="applied"
								<%="applied".equals(request.getAttribute("status"))?"selected":"" %>>Applied</option>
							<option value="shortlisted"
								<%="shortlisted".equals(request.getAttribute("status"))?"selected":"" %>>Shortlisted</option>
							<option value="rejected"
								<%="rejected".equals(request.getAttribute("status"))?"selected":"" %>>Rejected</option>
						</select>
					</div>
				</div>
			</form>
				...
				<div class="applicant-card">
					...
					<form method="post" action="UpdateApplicationStatusServlet"
						...
						<input type="hidden" name="appId" value="<%=a.getId() %>" /> <input
							...
							<%if("shortlisted".equalsIgnoreCase(request.getAttribute("status").toString())){ %>
							...
							<%} 
							   else if("rejected".equalsIgnoreCase(request.getAttribute("status").toString())){%>
